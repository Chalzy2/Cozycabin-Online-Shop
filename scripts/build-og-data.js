...
function run() {
  ...
  Object.keys(byId).forEach((id) => {
    ...
    if (!imgPath) {
      warnings.push(`"${title}" (${id}) has no image — it will use the default share image.`);
      missingImage++;
    }
  ...
  if (missingImage) console.log(`  (${missingImage} product(s) missing an image — see warnings above)`);
}
