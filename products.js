...
(function injectVariantCSS() {
  ...
  s.textContent = [
    '.cc-variant-row{display:flex;flex-wrap:wrap;gap:8px;margin:10px 0 6px;}',
    '.cc-variant-btn{display:flex;flex-direction:column;align-items:center;gap:4px;border:2px solid rgba(255,215,0,0.25);border-radius:10px;background:rgba(10,17,40,0.7);padding:6px 10px;cursor:pointer;transition:border-color .18s,background .18s;min-width:72px;}',
    '.cc-variant-btn:hover{border-color:rgba(255,215,0,0.6);background:rgba(255,215,0,0.07);}',
    '.cc-variant-btn.cc-var-active{border-color:#ffd700;background:rgba(255,215,0,0.13);}',
    '.cc-variant-thumb{width:52px;height:52px;border-radius:7px;object-fit:cover;display:block;pointer-events:none;}',
    '.cc-variant-label{font-size:10px;font-weight:700;color:#cbd5e1;text-align:center;line-height:1.2;max-width:68px;white-space:normal;word-break:break-word;}',
    '.cc-variant-btn.cc-var-active .cc-variant-label{color:#ffd700;}',
    '.cc-var-price-wrap{transition:opacity .18s;opacity:1;}',
    '.cc-var-price-wrap.cc-fading{opacity:0;}',
    '.cc-var-selected-name{display:inline-block;font-size:11px;font-weight:700;background:rgba(255,215,0,0.12);color:#ffd700;border:1px solid rgba(255,215,0,0.3);border-radius:20px;padding:3px 12px;margin-bottom:6px;letter-spacing:0.04em;}',
    '.cc-var-main-img{width:100%;border-radius:14px;object-fit:cover;display:block;aspect-ratio:1/1;transition:opacity .22s;}',
    '.cc-var-main-img.cc-img-fade{opacity:0;}',
    '.cc-var-img-wrap{position:relative;width:100%;margin-bottom:10px;border-radius:14px;overflow:hidden;background:#0a0f1e;}'
  ].join('');
  document.head.appendChild(s);
...
function renderVariantCard(product, index) {
  ...
  priceWrap.innerHTML =
    '<span class="new-price" id="cc-var-newp-' + index + '">KES ' + (firstVar.price || 0).toLocaleString() + '</span>' +
    '<span class="old-price" id="cc-var-oldp-' + index + '">KES ' + (firstVar.oldPrice || 0).toLocaleString() + '</span>' +
    '<span class="savings-badge" id="cc-var-badge-' + index + '"' + (pct > 0 ? '' : ' style="display:none;"') + '>' + (pct > 0 ? 'SAVE ' + pct + '%' : '') + '</span>';
  card.appendChild(priceWrap);
  ...
  buyBtn.textContent = '🛒 Order via WhatsApp';
  buyBtn.addEventListener('click', function() {
    var vi = (window._ccSelectedVariant && window._ccSelectedVariant[index] !== undefined) ? window._ccSelectedVariant[index] : 0;
  ...
  var details = document.createElement('details');
  details.className = 'details-box';
  details.innerHTML = '<summary>📋 More Details</summary><p style="margin-top:10px;">Premium quality product. Nationwide delivery available via G4S, Simba Coach, Tahmeed and more.</p>';
  card.appendChild(details);
...
(function injectSimilarCSS() {
  ...
  s.textContent = [
    '.cc-similar-wrap{margin-top:22px;padding-top:16px;border-top:1px solid rgba(255,215,0,0.12);}',
    '.cc-similar-heading{font-size:11px;font-weight:800;color:#ffd700;text-transform:uppercase;letter-spacing:0.12em;margin-bottom:12px;}',
    '.cc-similar-row{display:flex;gap:10px;overflow-x:auto;padding-bottom:6px;scrollbar-width:none;}',
    '.cc-similar-row::-webkit-scrollbar{display:none;}',
    '.cc-similar-item{flex:0 0 110px;cursor:pointer;border-radius:10px;overflow:hidden;background:rgba(10,17,40,0.7);border:1px solid rgba(255,215,0,0.15);transition:border-color .18s,transform .18s;}',
    '.cc-similar-item:hover{border-color:rgba(255,215,0,0.5);transform:translateY(-2px);}',
    '.cc-similar-item:active{transform:scale(0.97);}',
    '.cc-sim-img{width:100%;height:90px;object-fit:cover;display:block;}',
    '.cc-sim-info{padding:6px 7px 8px;}',
    '.cc-sim-title{font-size:10px;font-weight:700;color:#e2e8f0;line-height:1.3;margin-bottom:3px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;}',
    '.cc-sim-price{font-size:11px;font-weight:800;color:#ffd700;}'
  ].join('');
  document.head.appendChild(s);
})();
...
function ccGetSimilar(currentCategory, currentIndex) {
  ...
  sameCat.forEach(function(p, i) {
    ...
    var key = currentCategory + '-' + i;
    if (!seen[key]) { seen[key] = true; results.push({ product: p, category: currentCategory, catIndex: i }); }
  });
  if (results.length < 4) {
    var related = CC_RELATED[currentCategory] || [];
    for (var ri = 0; ri < related.length && results.length < 8; ri++) {
      ...
      for (var rj = 0; rj < relArr.length && results.length < 8; rj++) {
        var key2 = relCat + '-' + rj;
        if (!seen[key2]) { seen[key2] = true; results.push({ product: relArr[rj], category: relCat, catIndex: rj }); }
      }
    }
  }
  return results.slice(0, 8);
