...
window.showProducts = function(category) {
  ...
  /* Move container right after the currently open submenu */
  ...
  products[category].forEach(function(product, index) {
    ...
    (product.sizes || []).forEach(function(size) {
      sizesHTML +=
        '<button class="size-btn" onclick="selectSize(' + index + ', ' + size + ', this)">' +
        size + '</button>';
    });
    ...
    (product.colors || []).forEach(function(color) {
      colorsHTML +=
        '<button class="color-btn" onclick="selectColor(' + index + ', \'' + color + '\', this)">' +
        color + '</button>';
    });
    ...
    card.innerHTML =
      '<div class="product-gallery">' +
        '<img id="mainImage-' + index + '" src="' + product.images[0] + '" ' +
        'class="main-product-image" loading="lazy" alt="' + product.title + '">' +
      '</div>' +
      ...
      '<div class="sizes-box"><h4>Select Size</h4>' +
        '<div class="sizes-row" id="sizes-' + index + '">' + sizesHTML + '</div>' +
      '</div>' +
      '<div class="colors-box"><h4>Select Color</h4>' +
        '<div class="colors-row" id="colors-' + index + '">' + colorsHTML + '</div>' +
      '</div>' +
      '<p id="selection-hint-' + index + '" class="selection-hint"></p>' +
      '<button class="buy-btn" onclick="buyNow(' + index + ', \'' +
        product.title.replace(/'/g, "\\'") + '\', ' + product.price + ')">' +
        '🛒 Order via WhatsApp' +
      '</button>' +
      '<button class="cart-btn">Add to Cart</button>' +
