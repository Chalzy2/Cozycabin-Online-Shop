...
// ============================================================
//  COZYCABIN GA4 ANALYTICS — products.js integration
//  Tracks: view_item, product_click, whatsapp_order_click,
//          share, copy_link, add_to_cart, affiliate_signup,
//          search, contact_submit
//
//  Rules:
//  • Never throws — all wrapped in try/catch + gtag guard
//  • Never modifies existing logic — all patches are additive
//  • Never runs on admin pages (checks URL path)
//  • Admin dashboard, Firebase, Firestore — all untouched
// ============================================================
(function ccGA4() {

  // ── Guard: skip on admin pages ──────────────────────────────
  var _path = window.location.pathname || '';
  if (/admin|dashboard/i.test(_path)) return;

  // ── Safe gtag wrapper ──────────────────────────────────────
  function ccTrack(eventName, params) {
    try {
  ...
  // ── Expose for inline HTML calls (share buttons, forms) ────
  window.ccTrack = ccTrack;

  // ============================================================
  //  1. VIEW_ITEM — fires when a product card becomes visible
  //     Patches the existing IntersectionObserver via showProducts
  // ============================================================
  (function patchViewItem() {
    var _origShow = window.showProducts;
    if (typeof _origShow !== 'function') return;

    window.showProducts = function(category) {
      _origShow(category);

      // After cards are in DOM, watch for actual visibility
      if (!window.IntersectionObserver) return;
      var container = document.getElementById('products-container');
      if (!container) return;
      var cards = container.querySelectorAll('.product-card');
      ...
      // Reuse or create a per-session seen set (separate from Firestore one)
      var ga4Viewed = window._ccGA4Viewed || (window._ccGA4Viewed = {});

      var obs = new IntersectionObserver(function(entries) {
        entries.forEach(function(entry) {
          if (!entry.isIntersecting) return;
          var card  = entry.target;
          var title = card.querySelector('.product-title, h2, .pc-name, .card-title');
          var pName = (title ? title.textContent : '').trim();
          var pId   = (category + '_' + pName).replace(/\s+/g,'_').toLowerCase().substring(0,80);
          if (!pName || ga4Viewed[pId]) return;
          ga4Viewed[pId] = true;
          obs.unobserve(card);
  ...
  // ============================================================
  //  2. PRODUCT_CLICK — fires when a product card is tapped
  //     Bubble phase only — must NOT interfere with grid-btn /
  //     minor-btn / back-btn / buy-btn click handlers
  // ============================================================
  document.addEventListener('click', function(e) {
    try {
      // Never touch navigation or action buttons
      if (e.target.closest('.grid-btn') ||
          e.target.closest('.minor-btn') ||
          e.target.closest('.back-btn') ||
          e.target.closest('.buy-btn') ||
          e.target.closest('.cart-btn')) return;

      var card = e.target.closest('.product-card');
      if (!card) return;

      var title = card.querySelector('.product-title, h2, .pc-name');
      var pName = title ? title.textContent.trim() : '';

      var container = document.getElementById('products-container');
      var category  = container ? (container.getAttribute('data-current-category') || '') : '';

      if (pName) {
        ccTrack('product_click', {
          product_name: pName,
          category:     category
        });
      }
  ...
  // ============================================================
  //  3. WHATSAPP_ORDER_CLICK — fires when payment modal opens
  //     Patches openPaymentModal (which is already defined)
  // ============================================================
  (function patchOrderModal() {
    var _origOpen = window.openPaymentModal;
    ...
    window.openPaymentModal = function() {
      ...
      try {
        var order = window._pendingOrder || {};
        ccTrack('whatsapp_order_click', {
          product_name: order.title   || '',
          category:     order.productId ? order.productId.split('-')[0] : ''
        });
      } catch (e) {}
    };
  })();
  ...
  // ============================================================
  //  4. SHARE — fires on WhatsApp share, Facebook share
  //     Delegation on all share links in payment modal
  // ============================================================
  document.addEventListener('click', function(e) {
    try {
      ...
      var href = link.href || '';
      var order = window._pendingOrder || {};
      var pName = order.title || document.title || '';

      if (/wa\.me|whatsapp\.com\/send/i.test(href)) {
        // Only fire share (not order) for links that are NOT the order button
        if (!link.id || link.id !== 'wa-order-link') {
          ccTrack('share', { method: 'whatsapp', content_type: 'product', item_id: pName });
        }
      } else if (/facebook\.com\/sharer|fb\.me/i.test(href)) {
        ccTrack('share', { method: 'facebook', content_type: 'product', item_id: pName });
      }
    } catch (e2) {}
  });

  // ============================================================
  //  5. COPY_LINK — fires when copy-link button is used
  //     Patches cozyyCopy / copyText
  // ============================================================
  (function patchCopy() {
    var _origCopy = window.cozyyCopy || window.copyText;
    if (typeof _origCopy !== 'function') return;

    var _wrapper = function(text, btnId) {
      _origCopy(text, btnId);
      try {
        var order = window._pendingOrder || {};
        ccTrack('copy_link', {
          product_name: order.title || '',
    ...
    window.cozyyCopy = _wrapper;
    window.copyText  = _wrapper;
  })();

  // ============================================================
  //  6. ADD_TO_CART — fires on cart-btn click
  // ============================================================
  document.addEventListener('click', function(e) {
    try {
      ...
      var title = card ? card.querySelector('.product-title, h2, .pc-name') : null;
      var pName = title ? title.textContent.trim() : '';
      ccTrack('add_to_cart', {
        item_name: pName,
        quantity:  1
  ...
  // ============================================================
  //  7. AFFILIATE_SIGNUP — fires on affiliate registration form
  //     Watches for the form submit button or success message
  //     in affiliate-redesigned.html
  // ============================================================
  (function watchAffiliateSignup() {
    // Method A: patch the global ccRegisterAffiliate if it exists
    if (typeof window.ccRegisterAffiliate === 'function') {
      var _origReg = window.ccRegisterAffiliate;
      window.ccRegisterAffiliate = function() {
        _origReg.apply(this, arguments);
        ccTrack('affiliate_signup');
      };
    }

    // Method B: observe DOM for success message (covers async registration)
    if (!window.MutationObserver) return;
    var _signupFired = false;
    var observer = new MutationObserver(function(mutations) {
      if (_signupFired) return;
      mutations.forEach(function(m) {
        m.addedNodes.forEach(function(node) {
          if (!node.textContent) return;
          // Success messages typically say "registered", "success", "welcome"
          if (/registr|success|welcome|affiliate.*creat/i.test(node.textContent)) {
            _signupFired = true;
            ccTrack('affiliate_signup');
          }
        });
  ...
  // ============================================================
  //  8. SEARCH — fires when user submits a search query
  //     Watches search input / form on index.html
  // ============================================================
  (function watchSearch() {
    function attachSearch(input) {
      if (!input || input._ccGA4Search) return;
      input._ccGA4Search = true;

      // Fire on Enter key
      input.addEventListener('keydown', function(e) {
        if (e.key === 'Enter' && input.value.trim()) {
          ccTrack('search', { search_term: input.value.trim() });
        }
      });
      ...
      // Fire when a search button sibling is clicked
      var sibling = form
        ? form.querySelector('button[type=submit], .search-btn, #search-btn')
        : input.parentElement && input.parentElement.querySelector('button, .search-btn');
      if (sibling) {
    ...
    // Attach to any existing search inputs
    document.querySelectorAll('input[type=search], input[placeholder*=earch], #search-input, .search-input')
      ...
      new MutationObserver(function(muts) {
        muts.forEach(function(m) {
          m.addedNodes.forEach(function(node) {
            ...
            var inputs = [].concat(
              node.matches && node.matches('input') ? [node] : [],
              Array.from(node.querySelectorAll ? node.querySelectorAll('input[type=search], input[placeholder*=earch], #search-input, .search-input') : [])
            );
  ...
  // ============================================================
  //  9. CONTACT_SUBMIT — fires on contact form submission
  // ============================================================
  (function watchContactForm() {
    function attachContact(form) {
      if (!form || form._ccGA4Contact) return;
      form._ccGA4Contact = true;
      form.addEventListener('submit', function() {
        ccTrack('contact_submit');
      });
    }

    document.querySelectorAll('form.contact-form, form#contact-form, #contact form, .contact-section form')
      .forEach(attachContact);

    // Watch for dynamically added contact forms
    if (window.MutationObserver) {
      new MutationObserver(function(muts) {
        muts.forEach(function(m) {
          m.addedNodes.forEach(function(node) {
            if (node.nodeType !== 1) return;
            var forms = node.matches && node.matches('form') ? [node] : Array.from(node.querySelectorAll ? node.querySelectorAll('form') : []);
            forms.forEach(function(f) {
              if (/contact/i.test(f.id + ' ' + f.className)) attachContact(f);
            });
          });
        });
      }).observe(document.body, { childList: true, subtree: true });
  ...
  // ============================================================
  //  10. Store current category on container so product_click
  //      can read it (set whenever showProducts is called)
  // ============================================================
  (function trackCurrentCategory() {
    var _origShow2 = window.showProducts;
    if (typeof _origShow2 !== 'function') return;
    window.showProducts = function(category) {
      _origShow2(category);
      var container = document.getElementById('products-container');
      if (container) container.setAttribute('data-current-category', category || '');
    };
  })();
