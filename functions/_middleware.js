...
export async function onRequest(context) {
  ...
  // ── DEBUG: log every request for diagnosis ─────────────────
  console.log('[OG] url:', request.url);
  console.log('[OG] id param:', JSON.stringify(id));
  console.log('[OG] shop param:', JSON.stringify(shop), 'idx:', idx);
  console.log('[OG] ogData has id?', id ? (ogData[id] ? 'YES' : 'NO — not in og-data.json') : 'no id param');

  // ── Resolve a final product id from either ?id= or legacy ?shop=&idx= ──
  // NEVER fall back to Object.values(ogData)[0] — only use site default
  // when the id is truly missing or not found in og-data.json.
  let resolvedId = null;
  ...
  if (id) {
    if (ogData[id]) {
      ...
      // id param present but not in og-data.json — product may have been
      // added after the last build. Log clearly so you know to rebuild.
      console.log('[OG] WARNING: id "' + id + '" not found in og-data.json.');
      console.log('[OG] Fix: run  node scripts/build-og-data.js  then redeploy.');
    }
  } else if (shop) {
    if (LEGACY_LOOKUP[shop] && LEGACY_LOOKUP[shop][idx] !== undefined) {
      resolvedId = LEGACY_LOOKUP[shop][idx];
    } else {
      console.log('[OG] Legacy lookup miss: shop=' + shop + ' idx=' + idx);
    }
  }

  if (resolvedId) {
    const p = ogData[resolvedId];
    // Guard: if build-og-data.js produced a null image (product had no image
    // when build ran), fall back to DEFAULT_IMAGE — never serve a null og:image.
    ogTitle = p.title || `${SITE_NAME} — Quality Products`;
    ogDesc  = p.description || ogDesc;
    ogImage = (p.image && p.image !== 'null') ? p.image : DEFAULT_IMAGE;
    ogPrice = p.price || null;
    ogUrl   = p.url || request.url;
    console.log('[OG] MATCH: id=' + resolvedId);
    console.log('[OG] og:title:', ogTitle);
    console.log('[OG] og:image:', ogImage);
  } else if (shop) {
    // Category-level fallback — link has a category but no specific product
    ...
    console.log('[OG] Category fallback for shop=' + shop);
    console.log('[OG] og:title:', ogTitle);
    console.log('[OG] og:image:', ogImage);
  } else {
    // Homepage / no params — site-level default
    console.log('[OG] Homepage default');
    console.log('[OG] og:title:', ogTitle);
    console.log('[OG] og:image:', ogImage);
  }
